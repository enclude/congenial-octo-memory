"""Custom PySide6 widgets driven by design tokens (UI layer, may import Qt).

Companion of ``qt_theme.py``. Copy both into the application (for example as
``ui_theme.py`` and ``ui_widgets.py``); adjust the import below. Every widget
takes an optional ``tokens`` mapping; when omitted, the tokens of the theme
applied with ``qt_theme.apply_theme`` are used (fallback: built-in dark set).
...
Run ``python qt_widgets.py`` for a demo inspector; ``--screenshot PATH`` renders
it offscreen for review.
"""
...
try:  # theme module is optional so the widgets can be dropped in alone
    from qt_theme import RADIUS, SPACING, TOKENS, current_tokens, repolish
    _HAVE_THEME = True
except ImportError:  # pragma: no cover - fallback when qt_theme.py is absent
    _HAVE_THEME = False
...
# ---------------------------------------------------------------------------
# Demo: an inspector shaped like the Piro Overlay one
# ---------------------------------------------------------------------------

def _build_demo() -> QWidget:
    from PySide6.QtWidgets import (QComboBox, QMainWindow, QPlainTextEdit, QProgressBar,
                                   QScrollArea)

    pl = QLocale(QLocale.Polish)
    win = QMainWindow()
    win.setWindowTitle("qt_widgets demo")
    scroll = QScrollArea()
    scroll.setWidgetResizable(True)
    scroll.setFrameShape(QFrame.NoFrame)
    scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
    inspector = QWidget()
    inspector.setProperty("role", "inspector")
    col = QVBoxLayout(inspector)
    col.setContentsMargins(SPACING["sp_4"], SPACING["sp_4"], SPACING["sp_4"], SPACING["sp_4"])
    col.setSpacing(SPACING["sp_6"])

    # --- Wejście ------------------------------------------------------------
    sec_in = FormSection("Wejście")
    video = PathField("open", "Wideo (*.mp4 *.mov *.mkv)", "Przeciągnij plik wideo lub wybierz")
    video.set_path("C:/Users/jz/Videos/DJI_0042_trening_stage3.MP4", emit=False)
    sec_in.add_row("Wideo", video)
    source = SegmentedControl([("text", "Tekst"), ("id", "ID (API)")])
    source.set_value("id")
    sec_in.add_row("Źródło", source)
    timeline = QPlainTextEdit()
    timeline.setPlaceholderText("1: 2.81s | 2: 4.63s (+1.82s) | ...")
    timeline.setMaximumHeight(72)
    sec_in.add_row("Oś czasu", timeline)
    session_id = NumberField(1, 10_000_000, 1, 0, value=4213)
    fetch_row = QWidget()
    fr = QHBoxLayout(fetch_row)
    fr.setContentsMargins(0, 0, 0, 0)
    fr.setSpacing(SPACING["sp_2"])
    fr.addWidget(session_id, 2)
    fetch = QPushButton("Pobierz")
    fetch_trim = QPushButton("Pobierz i przytnij")
    for b in (fetch, fetch_trim):
        b.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Fixed)   # may shrink below text
    fr.addWidget(fetch, 1)
    fr.addWidget(fetch_trim, 2)
    sec_in.add_row("ID", fetch_row)
    msg_in = InlineMessage()
    msg_in.show_message("Pobrano sesję 4213: 18 strzałów, tor 3", "success")
    sec_in.add_widget_row(msg_in)
    col.addWidget(sec_in)

    # --- Synchronizacja i przycięcie ---------------------------------------
    sec_sync = FormSection("Synchronizacja i przycięcie")
    anchor = QComboBox()
    anchor.addItem("Sygnał startu", "start_signal")
    anchor.addItem("Pierwszy strzał", "first_shot")
    sec_sync.add_row("Typ kotwicy", anchor)
    det_row = QWidget()
    dr = QHBoxLayout(det_row)
    dr.setContentsMargins(0, 0, 0, 0)
    dr.setSpacing(SPACING["sp_2"])
    detect = QPushButton("Wykryj kotwicę")
    detect.setProperty("kind", "secondary")
    nextc = QPushButton("Następny kandydat")
    nextc.setProperty("kind", "ghost")
    busy_btn = QPushButton("Wykryj sygnał startu")
    set_busy(busy_btn, True, "Wykrywanie...")
    for b in (detect, nextc, busy_btn):
        b.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Fixed)
        dr.addWidget(b, 1)
    sec_sync.add_widget_row(det_row)
    t0 = NumberField(0, 100000, 0.05, 2, " s", 37.6, pl)
    sec_sync.add_row("Kotwica (czas)", t0, help_text="Klik na fali audio ustawia kotwicę.")
    trim_a = NumberField(0, 100000, 0.1, 1, " s", 32.6, pl, buttons=False)
    trim_b = NumberField(0, 100000, 0.1, 1, " s", 71.3, pl, buttons=False)
    sec_sync.add_pair_row("Przytnij od / do", trim_a, trim_b)
    bad = NumberField(0, 60, 0.5, 1, " s", 5.0, pl)
    bad.setInvalid(True)
    sec_sync.add_row("Margines końcowy", bad)
    msg_sync = InlineMessage()
    msg_sync.show_message("Koniec przycięcia wypada przed ostatnim strzałem.", "danger")
    sec_sync.add_widget_row(msg_sync)
    prog = QProgressBar()
    prog.setRange(0, 0)
    prog.setTextVisible(False)
    sec_sync.add_widget_row(prog)
    col.addWidget(sec_sync)

    # --- Wygląd nakładki ----------------------------------------------------
    sec_look = FormSection("Wygląd nakładki")
    lang = QComboBox()
    lang.addItem("Polski", "pl")
    lang.addItem("English", "en")
    sec_look.add_row("Język", lang)
    scale = NumberField(0.3, 5.0, 0.1, 1, "", 1.0, pl)
    sec_look.add_row("Rozmiar (skala)", scale, unit="x")
    pos = QComboBox()
    for key, label in (("bottom-left", "Lewy dolny róg"), ("bottom-right", "Prawy dolny róg"),
                       ("top-left", "Lewy górny róg"), ("top-right", "Prawy górny róg")):
        pos.addItem(label, key)
    sec_look.add_row("Pozycja", pos)
    off_x = NumberField(0, 8000, 1, 0, " px", 32, buttons=False)
    off_y = NumberField(0, 8000, 1, 0, " px", 32, buttons=False)
    sec_look.add_pair_row("Offset X / Y", off_x, off_y)
    sec_look.add_row("Tło panelu", ColorSwatchButton((0, 0, 0, 170)))
    sec_look.add_row("Tekst", ColorSwatchButton((255, 255, 255, 255)))
    sec_look.add_row("Akcent", ColorSwatchButton((255, 196, 0, 255)))
    sec_look.add_row("Obramowanie", ColorSwatchButton((255, 255, 255, 220)))
    sw = Switch("Plansza START")
    sw.setChecked(True)
    sec_look.add_row("Sygnał startu", sw)
    sw2 = Switch("Bez nakładki (tylko przycięcie)")
    sec_look.add_row("Tryb", sw2)
    col.addWidget(sec_look)
    ...
    render = QPushButton("Renderuj")
    render.setProperty("kind", "primary")
    render.setDefault(True)
    foot.addWidget(cancel)
    foot.addWidget(render)
    col.addLayout(foot)
    col.addStretch(1)

    scroll.setWidget(inspector)
    win.setCentralWidget(scroll)
    win._inspector = inspector   # exposed for the screenshot mode
    status_message(win.statusBar(), "Gotowe. Wczytano ustawienia pliku.", "success", 0)
    win.resize(600, 900)   # inspector width like the app's left pane at 150 %
    return win
...
def _main(argv: list[str]) -> int:
    ...
    mode = "light" if "--light" in argv else "dark"
    if _HAVE_THEME:
        from qt_theme import apply_theme, set_windows_dark_titlebar, setup_hidpi
        setup_hidpi()
    ...
    if shot:
        ...
        root_name, ext = os.path.splitext(shot)
        win._inspector.grab().save(root_name + "_full" + ext)   # whole inspector, no scrolling
