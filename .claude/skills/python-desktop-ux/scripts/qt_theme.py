"""Qt (PySide6) theme layer: tokens, QPalette, QSS, HiDPI, Windows integration.

Copy this module into the application as a UI-layer module (for example
``src/piro_overlay/ui_theme.py``). It MAY import PySide6 because it is part of
the UI layer; domain modules must never import it.

Source of truth for the values below: references/design-tokens.md. When the
document and this file disagree, the document wins; fix this file.

Usage in ``main()`` (order matters, see references/qt-pyside6.md §2):

    setup_hidpi()                       # before QApplication
    app = QApplication(sys.argv)
    load_app_fonts(resources.fonts_dir())
...
Standalone gallery:
    ...
    python qt_theme.py                  # interactive gallery, dark/light toggle
    python qt_theme.py --screenshot out.png   # headless (offscreen) capture
"""
...
# ---------------------------------------------------------------------------
# Gallery (manual / headless verification)
# ---------------------------------------------------------------------------

def _build_gallery(app: Any) -> Any:
    from PySide6.QtCore import Qt
    from PySide6.QtWidgets import (
        QCheckBox, QComboBox, QDoubleSpinBox, QFormLayout, QGroupBox, QHBoxLayout,
        QLabel, QLineEdit, QMainWindow, QPlainTextEdit, QProgressBar, QPushButton,
        QRadioButton, QScrollArea, QSlider, QSpinBox, QTabWidget, QVBoxLayout, QWidget,
    )

    win = QMainWindow()
    win.setWindowTitle("qt_theme gallery")
    central = QWidget()
    root = QVBoxLayout(central)
    root.setContentsMargins(SPACING["sp_4"], SPACING["sp_4"], SPACING["sp_4"], SPACING["sp_4"])
    root.setSpacing(SPACING["sp_3"])

    top = QHBoxLayout()
    title = QLabel("Galeria ostylowanych widżetów")
    title.setProperty("role", "title")
    top.addWidget(title)
    top.addStretch(1)
    toggle = QPushButton("Przełącz ciemny/jasny")
    toggle.setProperty("kind", "secondary")

    def _toggle() -> None:
        mode = "light" if app.property("theme_mode") == "dark" else "dark"
    ...
    lbls = QHBoxLayout()
    for role in ("section", "muted", "mono", "danger", "success", "warning", "info"):
        lb = QLabel(f"role={role}")
        lb.setProperty("role", role)
        lbls.addWidget(lb)
    lbls.addStretch(1)
    form.addRow("Role QLabel", _wrap(lbls))
    ...
    for kind in ("primary", "secondary", "ghost", "danger"):
        ...
        btns.addWidget(b)
    dis = QPushButton("Wyłączony")
    dis.setEnabled(False)
    btns.addWidget(dis)
    btns.addStretch(1)
    form.addRow("Rodzaje QPushButton", _wrap(btns))

    seg = QHBoxLayout()
    seg.setSpacing(SPACING["sp_1"])
    for i, txt in enumerate(("Tekst", "ID (API)")):
        b = QPushButton(txt)
        b.setCheckable(True)
        b.setChecked(i == 1)
        ...
        seg.addWidget(b)
    seg.addStretch(1)
    form.addRow("Segment", _wrap(seg))

    edit = QLineEdit()
    edit.setPlaceholderText("Zwykłe QLineEdit z podpowiedzią")
    form.addRow("QLineEdit", edit)
    bad = QLineEdit("37,6x")
    bad.setProperty("invalid", "true")
    form.addRow("invalid=true", bad)
    ro = QLineEdit("C:/video/session_0042.mp4")
    ro.setReadOnly(True)
    form.addRow("tylko do odczytu", ro)

    spins = QHBoxLayout()
    ds = QDoubleSpinBox()
    ds.setRange(0, 100000)
    ds.setDecimals(2)
    ds.setSuffix(" s")
    ds.setValue(37.6)
    ...
    isp.setValue(32)
    isp.setSuffix(" px")
    isp.setAlignment(Qt.AlignRight)
    spins.addWidget(isp)
    form.addRow("Pola liczbowe", _wrap(spins))

    combo = QComboBox()
    combo.addItem("Sygnał startu", "start_signal")
    combo.addItem("Pierwszy strzał", "first_shot")
    form.addRow("QComboBox", combo)

    checks = QHBoxLayout()
    c1 = QCheckBox("Zaznaczone")
    c1.setChecked(True)
    c2 = QCheckBox("Odznaczone")
    r1 = QRadioButton("Radio włączone")
    r1.setChecked(True)
    r2 = QRadioButton("Radio wyłączone")
    for w in (c1, c2, r1, r2):
        checks.addWidget(w)
    checks.addStretch(1)
    form.addRow("Check / radio", _wrap(checks))
    ...
    form.addRow("QProgressBar", prog)
    busy = QProgressBar()
    busy.setRange(0, 0)
    busy.setTextVisible(False)
    form.addRow("nieokreślony", busy)

    mono = QPlainTextEdit("1: 2.81s | 2: 4.63s (+1.82s) | 3: 6.10s (+1.47s)")
    mono.setMaximumHeight(64)
    form.addRow("QPlainTextEdit", mono)
    tabs.addTab(page, "Kontrolki")

    # --- tab 2: scroll area with group boxes -----------------------------
    scroll = QScrollArea()
    scroll.setWidgetResizable(True)
    inner = QWidget()
    inner.setProperty("role", "inspector")
    lay = QVBoxLayout(inner)
    lay.setContentsMargins(SPACING["sp_4"], SPACING["sp_2"], SPACING["sp_4"], SPACING["sp_4"])
    lay.setSpacing(SPACING["sp_6"])
    for name in ("Wejście", "Synchronizacja i przycięcie", "Wygląd nakładki", "Wyjście"):
        box = QGroupBox(name)
        f = QFormLayout(box)
        f.setHorizontalSpacing(SPACING["sp_2"])
        f.setVerticalSpacing(SPACING["sp_2"])
        f.addRow("Czas kotwicy", QDoubleSpinBox())
        f.addRow("Pozycja", QComboBox())
        f.addRow("Opcja", QCheckBox("Włącz"))
        lay.addWidget(box)
    lay.addStretch(1)
    scroll.setWidget(inner)
    tabs.addTab(scroll, "Sekcje")

    win.setCentralWidget(central)
    win.statusBar().showMessage("Komunikat paska stanu")
    win.resize(760, 640)
    win._tabs = tabs  # exposed for the screenshot mode
    return win


def _wrap(layout: Any) -> Any:
    from PySide6.QtWidgets import QWidget
...
def _main(argv: list[str]) -> int:
    ...
    if "--screenshot" in argv:
        ...
        if sys.platform == "win32":
            # The offscreen platform has no font database of its own on
            # Windows; point it at the system fonts or text renders as boxes.
    ...
    apply_theme(app, mode)
    win = _build_gallery(app)
    win.show()
    if shot:
        app.processEvents()
        ok = win.grab().save(shot)
        print(("saved " if ok else "FAILED ") + shot)
        # Second capture: the tab with QGroupBox sections inside a QScrollArea.
        win._tabs.setCurrentIndex(1)
        app.processEvents()
        root_name, ext = os.path.splitext(shot)
        win.grab().save(root_name + "_sections" + ext)
